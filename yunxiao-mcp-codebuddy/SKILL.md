...
description: 在 CodeBuddy IDE 中配置、验证和排查阿里云云效（Yunxiao）MCP。当需要接入 `https://openapi-rdc.aliyuncs.com/ai/mcp`、MCP 面板出现 `Needs authentication` / `OAuth authorization required` / `streamableHttp connect failed`、CLI 注册成功但 IDE Agent 看不到 server、server 可见但 tools 数为 0，或浏览器显示 `Authentication Successful` 而 IDE 仍不连通时使用。
---
